...
def _transe_query_vec(kge, qtype: str, known_eid: int, rid: int):
    """TransE-style structural query vector in entity space (robust to different KGE impls)."""
    import torch
    ...
    def _get_any(names):
        for name in names:
            if hasattr(kge, name):
                t = _pick_tensor(getattr(kge, name))
                if t is not None:
                    return t
        # fallback: search Embedding-like members
        for name, obj in getattr(kge, '__dict__', {}).items():
            if any(key in name.lower() for key in names):
                t = _pick_tensor(obj)
                if isinstance(t, torch.Tensor):
                    return t
        return None
    E = _get_any(['ent','entity','entities','ent_emb','entity_emb','entity_embedding','entity_embeddings'])
    R = _get_any(['rel','relation','relations','rel_emb','relation_emb','relation_embedding','relation_embeddings'])
    if E is None or R is None:
        raise AttributeError(f"Cannot locate TransE embeddings on kge: has={list(getattr(kge,'__dict__',{}).keys())[:50]}")
    evec = E[known_eid]
    rvec = R[rid]
    return evec + rvec if qtype == 'tail' else evec - rvec
...
def eval_retrieval(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if q2e_ckpt:
        ...
        if in_dim == 2 * q_txt_dim:
            ...
            # --- ablation: full / text_only / geo_only ---
            ablation = cfg.get('eval', {}).get('ablation', 'full')
            if ablation == 'text_only':
                q_geo = torch.zeros_like(q_geo)
            elif ablation == 'geo_only':
                q_txt = torch.zeros_like(q_txt)
            # --------------------------------------------
            # --- geo_scale: q = [q_txt, geo_scale * q_geo] ---
            geo_scale = cfg.get('eval', {}).get('geo_scale', 1.0)
            if geo_scale is None:
                geo_scale = 1.0
            q_geo = q_geo * float(geo_scale)
            # -------------------------------------------------
